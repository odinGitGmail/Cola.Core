...
public class ColaSnowFlake : IColaSnowFlake
{
    ...
    public ColaSnowFlake(SnowFlakeModel model)
    {
        ...
        if (model.DatacenterId > MaxDatacenterId || model.DatacenterId < 0)
            throw new Utils.ColaExceptionUtils(EnumException.SnowFlakeDatacenterIdLengthGtOrLt, MaxDatacenterId.ToString());
        if (model.WorkerId > MaxWorkerId || model.WorkerId < 0)
            throw new Utils.ColaExceptionUtils(EnumException.SnowFlakeWorkerIdLengthGtOrLt, MaxWorkerId.ToString());
